...
strategy._oauth2.getOAuthAccessToken = function getOAuthAccessToken(code, oldParams, done) {
  ...
  this._request('POST', this._getAccessTokenUrl(), { // eslint-disable-line no-underscore-dangle
...
strategy.userProfile = (accessToken, done) => {
  ...
  strategy._oauth2._request('POST', strategy._oauth2._getAccessTokenUrl(), { // eslint-disable-line no-underscore-dangle
    'Content-Type': 'application/x-www-form-urlencoded',
  }, querystring.stringify(params), null, (error, data) => {
    if (error) done(error);
    else {
      ...
      profile._raw = data; // eslint-disable-line no-underscore-dangle
      profile._json = results; // eslint-disable-line no-underscore-dangle
